  ...
  prefixArray1.forEach(function(value) {
    ...
    prefixArray2.forEach(function(value2) {
      ...
      if (value.prefix !== value2.prefix) {
        ...
        if (
          !prefixManifest.hasOwnProperty(value.prefix) ||
...
function checkFile(filePath) {
    ...
    taglibMatches.forEach(function(taglibMatch) {
      ...
      if (!fs.existsSync(taglibPath)) {
        taglibPathViolations.push({
          ...
          message: "cfimport taglib path {1} not found".replace(
            "{1}",
            taglibPath
          ),
          severity: "error"
      ...
      if (templatePath.substr(0, 1) !== "/") {
        ...
        if (!fs.existsSync(templatePath)) {
          templatePathViolations.push({
            ...
            message: "cfinclude/include template path {1} not found".replace(
              "{1}",
              templatePath
            ),
            severity: "error"
  ...
  var unusedPrefixViolations = comparePrefixArrays(
    prefixes,
    prefixUsages,
    'cfimported namespace prefix "{2}" not used',
    "warning"
  );
  var unimportedPrefixViolations = comparePrefixArrays(
    prefixUsages,
    prefixes,
